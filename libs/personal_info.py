...
class Personal_Info(object):
    def __init__(self, my_json='../data/private_info.json'):
        self.pi_path = os.path.abspath(os.path.join(os.path.dirname(os.path.realpath(__file__)), my_json))
        if not os.path.isfile(self.pi_path):
            raise NameError("settings not found " + os.path.abspath(self.pi_path))
        else:
            self._open_json()
            self._read_json()
            self._close_json()
